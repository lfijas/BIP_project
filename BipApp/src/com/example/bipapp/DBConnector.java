...
public class DBConnector {
	...
	public static void connect(String database) {
		try {
			...
			conn = DriverManager.getConnection("jdbc:jtds:sqlserver://" + host + ":" + port + ";user=" + username + ";password=" + password + ";database=" + database);
	...
	public static Connection connectWithCatelog(String database, String catelog) {
		try {
			Class.forName("net.sourceforge.jtds.jdbc.Driver");
			conn = DriverManager.getConnection("jdbc:mondrian:Jdbc=jdbc:jtds:sqlserver://" + host + ":" + port + ";jdbc.databaseName=" + database + ";Catalog=" + catelog + ";JdbcDrivers=net.sourceforge.jtds.jdbc.Driver;JdbcUser=" + username + ";JdbcPassword=" + password);
			System.out.println("DB connect success");
		} catch (SQLException e) {
			e.printStackTrace();
		} catch (ClassNotFoundException e) {
